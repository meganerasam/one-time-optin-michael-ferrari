const utils = require("../helpers/activeCampaignUtils.js");

// Find contact by email
//      If contact exists, update his/her tag and group
//      Else, create contact then update his/her tag and group

exports.oneTimeOptInActiveCamapaignBen = async (req, res) => {
    try {
        // Variables
        let tagId = 2; // 370 if for test;
        let listId = 1;
        ...
        if (tagAdded && listAdded) {
            ...
            res.writeHead(307, {
                Location: `https://trading.bourseensemble.com/gebs-w231019-optin-merci/`
            }).end();
        } else if (tagAdded && !listAdded) {
            res.status(400).send("Contact NOT ADDED to list BUT tag added successfully");
        } else if (!tagAdded && listAdded) {
            res.status(400).send("Contact added to list BUT tag NOT ADDED successfully");
        } else {
